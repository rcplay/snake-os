...
#ifdef CONFIG_CNS21XX_SPI_FLASH
...
static struct spi_flash_info spi_flash_bank[CONFIG_SYS_MAX_FLASH_BANKS];
...
void flash_enable_direct_access(void) {
	// Disable SPI
	SPI_CONFIGURATION_REG &= ~(0x1 << 31);
	// Enable SPI serial flash access through 0x30000000 region
	HAL_MISC_ENABLE_SPI_SERIAL_FLASH_BANK_ACCESS();
}

void flash_disable_direct_access(void) {
	// Disable SPI serial flash access through 0x30000000 region
	HAL_MISC_DISABLE_SPI_SERIAL_FLASH_BANK_ACCESS();
	// Enable SPI
	SPI_CONFIGURATION_REG |= (0x1 << 31);
...
static ulong flash_get_size(vu_long *addr, flash_info_t *info)
{
	...
	u16 device_id;
	int i;
	ulong base = CONFIG_SYS_FLASH_BASE;
	...
	case EN_25F80:
		printf("Flash Manufacturer: EON\n");
		spi_flash_bank[0].sectors = EN_25F80_SECTOR_NUM;
		spi_flash_bank[0].sector_size = SPI_FLASH_SECTOR_SIZE;
		spi_flash_bank[0].pages = EN_25F80_PAGE_NUM;
		spi_flash_bank[0].page_size = SPI_FLASH_PAGE_SIZE;
		info->flash_id = FLASH_MAN_EON + EN_25F80_DEVICE_ID;
		info->sector_count = 64;
		info->size = 0x100000;
		memset(info->protect, 0, 64);
		for (i = 0; i < info->sector_count; i++) {
...
int write_buff(flash_info_t * info, uchar * src, ulong addr, ulong cnt)
{
	...
	u32 prog_size;
	u32 prog_size_total;
	u32 dest = addr - CONFIG_SYS_FLASH_BASE;
...
#endif // CONFIG_CNS21XX_SPI_FLASH
