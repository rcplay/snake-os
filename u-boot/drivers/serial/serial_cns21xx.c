...
//#include <asm/arch/star_system.h>
#include <serial.h>
...
void star_serial_config_baud_rate(u32 uart_port_no, u32 baud_rate, u32 input_clock)
{
...
#endif
	*((u32 volatile *)0x800004) = baud_rate;
	*((u32 volatile *)0x800008) = input_clock;

	updated_input_clock = input_clock;
	*((u32 volatile *)0x800000) = 0xFF00FFEB;
	
	//baud_rate_divisor = 39;
	baud_rate_divisor = updated_input_clock / (16 * baud_rate);
	*((u32 volatile *)0x800000) = 0xFF00FFEC;
	...
	*((u32 volatile *)0x800000) = 0xFF00FFFB;
}
...
void star_serial_hw_init(void)
{
	...
	PWRMGT_SOFTWARE_RESET_CONTROL_REG &= ~(1 << PWRMGT_UART1_SOFTWARE_RESET_BIT_INDEX);
	PWRMGT_SOFTWARE_RESET_CONTROL_REG |=  (1 << PWRMGT_UART1_SOFTWARE_RESET_BIT_INDEX);
	HAL_MISC_ENABLE_UART0_ACT0_PIN();
	HAL_MISC_ENABLE_UART1_ACT1_PIN();
	HAL_MISC_ENABLE_UART1_PINS();
#endif
}

static void cns21xx_serial_putc(const char);

static void cns21xx_serial_setbrg(void)
{
	DECLARE_GLOBAL_DATA_PTR;
	*((u32 volatile *)0x800004) = 0;

	*((u32 volatile *)0x800000) = 0xFF00FFFA;

	star_serial_hw_init();

	*((u32 volatile *)0x800000) = 0xFF00FFEA;
	star_serial_config_baud_rate(0, gd->baudrate, UART_INPUT_CLOCK);
	*((u32 volatile *)0x800000) = 0xFF00FFFC;

	star_serial_config_line_control(0, WORD_EIGHT_BITS, NONE_PARITY, ONE_STOP_BIT);
	star_serial_config_fifo(0, TX_RX_FIFO_ENABLE, 8);
	*((u32 volatile *)0x800000) = 0xFF00FFFD;
 ...
 *
 */
static int cns21xx_serial_init(void)
{
	serial_setbrg();
 ...
 */
static int cns21xx_serial_getc(void)
{
	char rx_data;
 ...
 */
static void cns21xx_serial_putc(const char c)
{
 ...
 */
static int cns21xx_serial_tstc(void)
{
	return (HAL_UART_CHECK_RX_DATA_READY(CONFIG_CONSOLE_UART_PORT));
}


static struct serial_device cns21xx_serial_drv = {
	.name	= "cns21xx_serial",
	.start	= cns21xx_serial_init,
	.stop	= NULL,
	.setbrg	= cns21xx_serial_setbrg,
	.putc	= cns21xx_serial_putc,
	.puts	= default_serial_puts,
	.getc	= cns21xx_serial_getc,
	.tstc	= cns21xx_serial_tstc,
};

void cns21xx_serial_initialize(void)
{
	serial_register(&cns21xx_serial_drv);
}

__weak struct serial_device *default_serial_console(void)
{
	return &cns21xx_serial_drv;
}
